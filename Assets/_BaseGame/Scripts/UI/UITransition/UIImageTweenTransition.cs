...
public class UIImageTweenTransition : TweenTransition
{
    ...
    {
        Progress = 0;
        MainTween = DOTween.To(this.GetDelegate<DOGetter<float>>(nameof(GetProgress)),
            this.GetDelegate<DOSetter<float>>(nameof(SetProgress)), 1, Duration)
            .SetDelay(Delay)
            .SetEase(Ease.Linear)
            .OnUpdate(this.GetDelegate<TweenCallback>(nameof(ChangeSprite)));
